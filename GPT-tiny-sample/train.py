...
def train_step(model,data_loader,optimizer,criterion,clip=1,print_every=None):  #每一个eopch的训练
    ...
    #for i, (dec_inputs, dec_outputs) in enumerate(data_loader): #dec_inputs: [b, tgt_len] , dec_outputs: [b, tgt_len]
        optimizer.zero_grad()
...
def train(model,data_loader):
    ...
    for epoch in range(epochs):
        ...
        train_loss = train_step(model, data_loader, optimizer, criterion, CLIP, print_every=10) #训练一个epoch
        end_time = time.time()

        torch.save(model.state_dict(), 'GPT2.pt') #保存模型权重
