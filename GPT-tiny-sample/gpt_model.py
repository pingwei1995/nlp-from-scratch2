...
device = torch.device("cpu") #指定用哪块gpu
...
# 上三角矩阵mask，这是因为用当前信息预测下一个字的时候，是无法看到后续的信息的。具体参考https://blog.csdn.net/qq_44833392/article/details/128499220?ops_request_misc=%257B%2522request%255Fid%2522%253A%2522167810557816800211574507%2522%252C%2522scm%2522%253A%252220140713.130102334.pc%255Fall.%2522%257D&request_id=167810557816800211574507&biz_id=&utm_medium=distribute.pc_search_result.none-task-code-2~all~first_rank_ecpm_v1~rank_v31_ecpm-2-128499220-0-null-null.142^v73^control,201^v4^add_ask,239^v2^insert_chatgpt&utm_term=%E4%B8%8A%E4%B8%89%E8%A7%92%E7%9F%A9%E9%98%B5mask
...
class ScaledDotProductAttention(nn.Module): #计算Q和K的相似度矩阵，然后乘V。对应笔记里的图
    ...
    def forward(self, Q, K, V, attn_mask): #前三者形状相同[b, n_heads, tgt_len, d_k=64]，attn_mask:[b, n_heads, tgt_len, tgt_len]
...
class MultiHeadAttention(nn.Module): #多头注意力机制
    ...
    def forward(self, input_Q, input_K, input_V, attn_mask): #前三者形状相同，都是[b, tgt_len, d_model]  , attn_mask: [b, tgt_len, tgt_len]
...
class GPT(nn.Module):
    ...
    def answer(self, sentence): #sentence是人输入的字符串  [n]   n是句子有多少个字符
